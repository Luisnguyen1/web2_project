...
<?php 
    if (isset($_REQUEST['submit'])){
        $taikhoan = $_REQUEST['inputEmail'];
        $matkhau = $_REQUEST['inputPassword'];
        $sql = "SELECT * FROM taikhoan WHERE email = '$taikhoan' AND matkhau = '$matkhau'";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            echo "<script>alert('Đăng nhập thành công');</script>";
            $sql = "SELECT * FROM khachhang WHERE email = '".$taikhoan."'";
            $result = $conn->query($sql);
            if ($result->num_rows > 0) {
                ...
                header("Location: http://localhost/web2-project/index.php");
                die();
            } else {
                header("Location: http://localhost/web2-project/admin/index.php");
                die();
            }
        } else{
